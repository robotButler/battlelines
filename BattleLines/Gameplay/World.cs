...
namespace BattleLines
{
    ...
    public class World : IDisposable
    {
        ...
        /// <summary>
        /// List of obstacle units in the game world
        /// </summary>
        private List<Rectangle> obstacleBlocks = new List<Rectangle>();
            ...
            NetworkSession networkSession)
        {
            ...
            particleEffectManager.RegisterParticleEffect(
                ...
                "Content/Particles/laserExplosion.xml", 40);
            particleEffectManager.RegisterParticleEffect(
                ParticleEffectType.MineExplosion,
                "Content/Particles/mineExplosion.xml", 8);
            particleEffectManager.RegisterParticleEffect(
                ParticleEffectType.RocketExplosion,
                "Content/Particles/rocketExplosion.xml", 24);
            particleEffectManager.RegisterParticleEffect(
                ParticleEffectType.RocketTrail,
                "Content/Particles/rocketTrail.xml", 16);
            particleEffectManager.RegisterParticleEffect(
                ParticleEffectType.ShipExplosion,
                "Content/Particles/shipExplosion.xml", 4);
            particleEffectManager.RegisterParticleEffect(
                ParticleEffectType.ShipSpawn,
                "Content/Particles/shipSpawn.xml", 4);
            ...
            int widthInUnits = dimensions.Width / barrierSize;
            int heightInUnits = dimensions.Height / barrierSize;
            for (int i = 0; i < obstacleCount; i++)
            {
                int positionWidth = (int)RandomMath.RandomBetween(0f, widthInUnits);
                int positionHeight = (int)RandomMath.RandomBetween(0f, heightInUnits);
                obstacleBlocks.Add(new Rectangle(dimensions.X + positionWidth * barrierSize,
                    dimensions.Y + positionHeight * barrierSize, barrierSize, barrierSize));
            }

            cameraBox = new Vector4(-1f, 1f, -1f, 1f);
        ...
        private void CenterCameraOnMouse()
        {
            MouseState mouseState = Mouse.GetState();
            Vector2 cursorPos = new Vector2(mouseState.X, mouseState.Y);
            Vector2 cameraCenter = new Vector2((cameraBox.X + cameraBox.Y) / 2f,
                                               (cameraBox.Z + cameraBox.W) / 2f);
            Vector2 centerDiff = cursorPos - cameraCenter;

            cameraBox.X += centerDiff.X;
            cameraBox.Y += centerDiff.X;
            cameraBox.Z += centerDiff.Y;
            cameraBox.W += centerDiff.Y;
        }
        ...
        public void Draw(float elapsedTime, Vector2 center)
        {
            ...
            if (InputManager.IsActionTriggered(InputManager.Action.ZoomOut))
            {
                cameraBox *= 1.1f;
                CenterCameraOnMouse();
            }
            else if (InputManager.IsActionTriggered(InputManager.Action.ZoomIn))
            {
                cameraBox *= 0.9f;
                CenterCameraOnMouse();
            }

            Matrix transform = Matrix.CreateOrthographicOffCenter(cameraBox.X, cameraBox.Y, cameraBox.Z, cameraBox.W, 1f, 0f);
            ...
            foreach (Rectangle rectangle in obstacleBlocks)
            {
                spriteBatch.Draw(horizontalBarrierTexture, rectangle, Color.White);
            }
